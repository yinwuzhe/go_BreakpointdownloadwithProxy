...
int main(int argc, char *argv[]) {
    ...
    if (local_port < 0) {
        printf("Syntax: %s -l local_port -d remote_host -p remote_port [-i \"input parser\"] [-o \"output parser\"]\n", argv[0]);
        return 0;
    }
    ...
    printf("proxy start successfully\n");
...
int parse_options(int argc, char *argv[]) {
    ...
    while ((c = getopt(argc, argv, "l:d:p:i:o:h")) != -1) {
        switch(c) {
            case 'l':
            ...
            case 'o':
                ...
                break;
	    case 'h':
		printf("./proxy_58 -l local_port -d remote_host -p remote_port [-i input parser] [-o output parser]\n");
	        exit(0);
	        break;
        }
    }
